...
class AstMaker extends AbstractParseTreeVisitor<AST> implements implVisitor<AST> {
	...
	public AST visitLoopStuff(implParser.LoopStuffContext ctx) {
		return null;
	}
	...
	public AST visitForLoopStuff(implParser.ForLoopStuffContext ctx) {
		LoopStuff l = (LoopStuff) visit(ctx.l);
		Command body = (Command) visit(ctx.p);
		return new ForLoopStuff(l, body);
	}

	public AST visitIfStatement(implParser.IfStatementContext ctx) {
		Condition c = (Condition) visit(ctx.c);
		Command body = (Command) visit(ctx.p);
		Command elseBody = (Command) visit(ctx.pelse);
		return new IfStatement(c, body, elseBody);
	}

	public AST visitConditional(implParser.ConditionalContext ctx) {
		Expr v1 = (Expr) visit(ctx.e1);
		Expr v2 = (Expr) visit(ctx.e2);
		String v3 = ctx.comp.getText();
		return new Conditional(v1, v2, v3);
	}
}
